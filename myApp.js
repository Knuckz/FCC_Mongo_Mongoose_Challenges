...
var createAndSavePerson = function(done) {
  let newPerson = new Person({
    name: "Austin Myers",
    age: 28,
    favoriteFoods: ['chicken', 'pasta', 'cereal']
  });
  
  //The call back being passed in lets us know when we are done saving
  //could even use 
  newPerson.save()
    .then((data) => done(null, data) )
    .catch((err) => done(err) )
  //could also use async await with try and catch
  // try {
...
var findEditThenSave = async function(personId, done) {
  ...
  try {
      let selectedPerson = await Person.findById(personId);
      selectedPerson.favoriteFoods.push(foodToAdd);
      done(null, await selectedPerson.save());
  } catch(error) {
    done(error);
  }
};
...
var findAndUpdate = function(personName, done) {
  ...
  Person.findOneAndUpdate({ name: personName }, {age: ageToSet}, { new: true, useFindAndModify: false }, (error, data) => {
    if (error) {
      done(error);
    }
    console.log(data);
